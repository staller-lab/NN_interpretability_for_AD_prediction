...
do
    seed=$(($seed - $x))
    out_model=../model_data/${model}model_b10_e${e}_L0.0001_sMinMax_c1_k${K}_wp${wp}_ParaReLU_hSplit_seed${seed}_weightInit
    out_folder=../results/${model}model_b10_e${e}_L0.0001_sMinMax_c1_k${K}_wp${wp}_ParaReLU_hSplit_seed${seed}_weightInit
    echo $out_model

    python Model.py -f $train_file -v $val_file  -i -b 10 -l 0.0001 -e $e -s MinMaxScaler -c 1 -k $K -o $out_model -m $model -wp $wp -seed $seed
    rm -rf $out_folder
