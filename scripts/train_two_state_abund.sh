...
do
    out_model=../model_data/${model}model_b10_e${e}_L0.0001_sMinMax_a${A}_c1_k${K}_ak${ak}_ParaReLU_${np}PosKLoss_${wp}PosWeightLoss_${ap}actLoss_hSplit_hv${hill_val}_seed${seed}_weightInit
    out_folder=../results/${model}model_b10_e${e}_L0.0001_sMinMax_a${A}_c1_k${K}_ak${ak}_ParaReLU_${np}PosKLoss_${wp}PosWeightLoss_${ap}actLoss_hSplit_hv${hill_val}_seed${seed}_weightInit
    echo $out_model

    python Model.py -f $train_file -v $val_file  -i -b 10 -l 0.0001 -e $e -s MinMaxScaler -a $A -c 1 -k $K -ak $ak -o $out_model -m $model -np $np -wp $wp -ap $ap -hv $hill_val -seed $seed
    rm -rf $out_folder
