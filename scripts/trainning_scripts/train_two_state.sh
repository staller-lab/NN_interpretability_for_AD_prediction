...
do
    out_model=../../model_data/${model}model_b10_e${e}_L0.0001_sMinMax_a${A}_c1_k${K}_ParaReLU_${np}PosKLoss_${wp}PosWeightLoss_hSplit_hv${hill_val}_weightInit_v2
    out_folder=../../results/${model}model_b10_e${e}_L0.0001_sMinMax_a${A}_c1_k${K}_ParaReLU_${np}PosKLoss_${wp}PosWeightLoss_hSplit_hv${hill_val}_weightInit
    echo $out_model

    python ../../Model/Model.py -f $train_file -v $val_file  -i -b 10 -l 0.0001 -e $e -s MinMaxScaler -a $A -c 1 -k $K -o $out_model -m $model -np $np -wp $wp -hv $hill_val 
    #rm -rf $out_folder
