...
results_file="../../results/results_simple_abund_test_largest_LR.csv"
...
do
    seed=$(($seed - $x))
    out_model=../../model_data/${model}model_b10_e${e}_L0.01_sMinMax_c1_k${K}_wp${wp}_ParaReLU_hSplit_seed${seed}_weightInit_v2
    out_folder=../../results/${model}model_b10_e${e}_L0.01_sMinMax_c1_k${K}_wp${wp}_ParaReLU_hSplit_seed${seed}_weightInit_v2
    echo $out_model

    python ../../Model/Model.py -f $train_file -v $val_file -t $test_file -i -b 10 -l 0.01 -e $e -s MinMaxScaler -c 1 -k $K -o $out_model -m $model -wp $wp -seed $seed
    #rm -rf $out_folder
