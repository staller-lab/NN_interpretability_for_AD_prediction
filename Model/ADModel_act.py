...
class ADModel_act(torch.nn.Module):
    """
    Class for implementing the GFP SimpleNN. 
    Code to create and train a NN that predicts GFP expression 
    ...
    def forward(self,x): 
        ...
        --------
        torch.Tensor
            A tensor containing activity predictions. 
        """

        x = x.float()

        # Predicting activity
        activitiy = self.conv1(x) # convolutional filter
        activitiy = self.activate(activitiy) # activation function
        ...
        activitiy = self.linear1(activitiy) # linear layer
        activitiy = self.activate(activitiy) # activation function
    ...
    def train_model(self, train_dataloader, val_dataloader, epochs, device, learning_rate, outfile, weight_pen=0):
        ...
        for e in range(self.num_epochs):
            ...
            for X,y in train_dataloader:
                ...
                # Compare predicted value with measured activity value
    ...
    def get_actual_and_predicted(self,dataloader,device,filepath, save_file=True):
        ...
        for X,y in dataloader:
            ...
            X = X.to(torch.float32) # Added because was getting error about diff type than internal weights
    ...
    def get_losses(self,outfile):
        ...
        # Plots tain and validation losses
