...
from Utility import one_hot_to_sequence, HillActivation, NormalizedHillActivation
...
class ADModel_two_state_abund(ADModel_two_state):
    ...
    activate (1-4) : torch.nn.ReLU or torch.nn.LeakyReLU
        Leaky ReLU or ReLU activation functions
...
class ADModel_two_state_abund_hill_normalized(ADModel_two_state):
    """
    Class for implementing the two state BiophysicalNN. 
    ...
    def __init__(self,input_shape,transformation,kernel_size=10,outchannel=1,relu = True, hill_val=1, abund_k=15, seed=25):
        ...
        super(ADModel_two_state_abund_hill_normalized, self).__init__(input_shape, transformation, kernel_size, outchannel, relu, hill_val) # initalizing parent class (torch.nn.Module)
        torch.manual_seed(seed)
        ...
        if transformation == "Linear":
            ...
            self.transf = NormalizedHillActivation(hill_val)
            self.transf.reset_parameters()
        else:
            print("Not a valid transformation, defaulting to Linear")
            self.transf = torch.nn.Linear(1, 1)
...
class ADModel_two_state_abund_frozen(ADModel_two_state):
    """
    Class for implementing the two state BiophysicalNN. 
    ...
    def __init__(self,input_shape,transformation,kernel_size=10,outchannel=1,relu = True, hill_val=1, abund_k=15, seed=25):
        ...
        super(ADModel_two_state_abund_frozen, self).__init__(input_shape, transformation, kernel_size, outchannel, relu, hill_val) # initalizing parent class (torch.nn.Module)
        torch.manual_seed(seed)


        self.input_shape = input_shape
        self.relu = relu
        self.abund_k = 15 #abund_k
        self.transformation = transformation
        self.kernel_size = kernel_size
        self.outchannel = outchannel
        self.hill_val = hill_val
        ...
        # Parameters for convolutional and linear layers
        kernel_size = (40,20) # (sequence position, amino acid)
        conv_out_height =self.input_shape[1] - kernel_size[0] + 1
        conv_out_width = self.input_shape[2] - kernel_size[1] + 1
        linear_input_size = conv_out_height * conv_out_width * self.outchannel
        
        # K size corresponding to weight dict
        self.conv1 = torch.nn.Conv2d(in_channels=1, out_channels=self.outchannel, kernel_size=kernel_size)
        torch.nn.init.kaiming_normal_(self.conv1.weight, mode='fan_in', nonlinearity='leaky_relu') 
        torch.nn.init.constant_(self.conv1.bias, 0) 
        ...
        model = "../../model_data/two_state_abundmodel_b10_e200_L0.001_sMinMax_aHill_c1_k40_ak15_LeakyReLU_0.5PosKLoss_0.1PosWeightLoss_hSplit_hv1_seed1793115_weightInit_v2"
        self.load_state_dict(torch.load(f"{model}.pth")) 

        # Rewriting with correct K size
        kernel_size = (self.kernel_size,20) # (sequence position, amino acid)
        conv_out_height =self.input_shape[1] - kernel_size[0] + 1
        conv_out_width = self.input_shape[2] - kernel_size[1] + 1
        linear_input_size = conv_out_height * conv_out_width * self.outchannel
    ...
    def train_model_abund(self,train_dataloader,val_dataloader,epochs,device,learning_rate,outfile, neg_pen, weight_pen):
        """
        We don't want to train abund at all
        """
